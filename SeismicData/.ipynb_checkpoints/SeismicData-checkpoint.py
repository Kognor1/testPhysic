...
class SeismicData():
    ...
    def get_spchere(self,CDP_X,CDP_Y,radius):
        ...
        return res
#         idx = (idx["IDX"].values[0])
#         start_radius = idx -radius
#         end_radius = idx + radius+1
        
#         print(start_radius,end_radius,len(self.head))
#         if(start_radius<0):
#             start_radius=0
#         if(end_radius>len(self.head)-1):
#             end_radius=len(self.head)-1
#         return pd.DataFrame(self.head[start_radius:end_radius])
